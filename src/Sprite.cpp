...
Sprite::Sprite(const sf::Texture& texture, size_t sheetX, size_t sheetY, uint32_t frameCount, bool repeat, uint16_t speed)
	:_frameCount(frameCount), _repeat(repeat), _speed(speed), _sheetX(sheetX), _sheetY(sheetY)
{
...
const sf::Sprite& Sprite::get() const
{
	return _sprite;
}